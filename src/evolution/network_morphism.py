...
class NetworkMorphism:
    ...
    def can_morph(self, parent_genome: Dict, child_genome: Dict) -> bool:
        ...
        if (parent_hyper['num_cells'] != child_hyper['num_cells'] or
            parent_hyper['init_channels'] != child_hyper['init_channels']):
            return False
    ...
    def morph_weights(self, parent_genome: Dict, child_genome: Dict, 
                     parent_model: nn.Module, child_model: nn.Module) -> nn.Module:
        """
        ...
        for diff in differences:
            ...
            child_state_dict = self._morph_operation(
                parent_state_dict, child_state_dict,
                cell_type, node_idx, connection_idx,
                parent_op, child_op
            )
    ...
    def _find_genome_differences(self, parent: Dict, child: Dict) -> List[Tuple]:
        ...
        for i, (parent_gene, child_gene) in enumerate(zip(parent['normal_cell'], child['normal_cell'])):
        ...
        for i, (parent_gene, child_gene) in enumerate(zip(parent['reduction_cell'], child['reduction_cell'])):
            if parent_gene[1] != child_gene[1]:  # Operation difference
                node_idx = i // 2
                connection_idx = i
    ...
    def _morph_operation(self, parent_state_dict: Dict, child_state_dict: Dict,
                        cell_type: str, node_idx: int, connection_idx: int,
                        parent_op: int, child_op: int) -> Dict:
        """
    ...
    def _is_identity_morphism(self, parent_op: str, child_op: str) -> bool:
        ...
        identical_pairs = [
            ...
            ('inv_bottleneck_2', 'inv_bottleneck_4'),  # Same structure, different expansion
        ]
    ...
    def _is_conv_morphism(self, parent_op: str, child_op: str) -> bool:
        ...
        return any(op in parent_op for op in conv_ops) and any(op in child_op for op in conv_ops)
    
    def _transfer_identical_ops(self, parent_state: Dict, child_state: Dict, 
                               pattern: str) -> Dict:
        """Transfer weights for identical operation structures."""
        for key in parent_state:
            if pattern in key:
                if key in child_state and parent_state[key].shape == child_state[key].shape:
                    child_state[key] = parent_state[key].clone()
        
        return child_state
    
    def _morph_conv_operations(self, parent_state: Dict, child_state: Dict,
                              pattern: str, parent_op: str, child_op: str) -> Dict:
        ...
        if '3x3' in parent_op and '5x5' in child_op:
            ...
            child_state = self._morph_kernel_size(parent_state, child_state, 
                                                 pattern, 3, 5)
        ...
        if 'sep_conv' in parent_op and 'group_conv' in child_op:
            child_state = self._morph_depthwise_to_grouped(parent_state, child_state, pattern)
        
        return child_state
    
    def _morph_kernel_size(self, parent_state: Dict, child_state: Dict,
                          pattern: str, from_size: int, to_size: int) -> Dict:
            ...
            if pattern in key and 'weight' in key:
                ...
                if len(parent_weight.shape) == 4:  # Conv weight
                    if from_size < to_size:
                        ...
                        new_weight = F.pad(parent_weight, 
                                          (pad, pad, pad, pad),
                    ...
                    if key in child_state and new_weight.shape == child_state[key].shape:
                        child_state[key] = new_weight.clone()
        
        return child_state
    
    def _morph_depthwise_to_grouped(self, parent_state: Dict, child_state: Dict,
                                   pattern: str) -> Dict:
            ...
            if pattern in key and 'weight' in key:
                ...
                if len(parent_weight.shape) == 4:
                    ...
                    if parent_weight.size(1) == 1:  # Depthwise
                        ...
                        if key in child_state and repeated_weight.shape == child_state[key].shape:
                            child_state[key] = repeated_weight.clone()
        
        return child_state
    
    def _partial_weight_transfer(self, parent_state: Dict, child_state: Dict,
                                pattern: str) -> Dict:
            ...
            if pattern in key:
                if key in child_state:
                    ...
                    if parent_tensor.shape == child_tensor.shape:
                        ...
                        min_shape = [min(p, c) for p, c in 
                                   zip(parent_tensor.shape, child_tensor.shape)]
...
class MorphismEnhancedEvolution:
    ...
    def evaluate_with_morphism(self, population: List[Dict], 
    ...
    def _find_morphism_parent(self, child_genome: Dict) -> Tuple[Optional[Dict], Optional[nn.Module]]:
        ...
        for parent_key, parent_data in self.morphism.weight_cache.items():
            ...
            if self.morphism.can_morph(parent_genome, child_genome):
                similarity = self._calculate_genome_similarity(parent_genome, child_genome)
    ...
    def _evaluate_with_inheritance(self, child_genome: Dict, 
                                 parent_genome: Dict,
