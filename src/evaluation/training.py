...
class TrainingProtocol:
    ...
    def search_phase_training(self, model: nn.Module, 
        ...
        """
        return self._train_model(model, train_loader, val_loader, 
                               self.search_hparams, phase="search")
    
    def final_evaluation(self, model: nn.Module,
                        train_loader: DataLoader, 
        ...
        for i, seed in enumerate(seeds):
            ...
            accuracy = self._train_model(model_copy, train_loader, test_loader,
                                       self.eval_hparams, phase="evaluation")
            end_time = time.time()
    ...
    def _train_model(self, model: nn.Module, 
        ...
        for epoch in range(hparams['epochs']):
            ...
            if phase == "search" and epoch % 5 == 0:
                print(f"Search Epoch {epoch}/{hparams['epochs']}, "
                      f"Accuracy: {accuracy:.2f}%")
            elif phase == "evaluation" and epoch % 50 == 0:
                print(f"Evaluation Epoch {epoch}/{hparams['epochs']}, "
                      f"Accuracy: {accuracy:.2f}%")
    ...
    def evaluate_architecture(self, genome: Dict, 
        ...
        if dataset == 'cifar10':
            if phase == 'search':
                ...
                train_loader, _, test_loader = dataset_manager.get_cifar10_loaders(cutout=True)
        elif dataset == 'cifar100':
            train_loader, test_loader = dataset_manager.get_cifar100_loaders(cutout=True)
