...
class ArchitectureGraph:
    ...
    def genome_to_graph(self, genome: Dict[str, Any]) -> Data:
        ...
        node_features.append(
            [0, 0, hyperparams["init_channels"], hyperparams["init_channels"]]
        )  # Input 1
        node_features.append(
            [0, 0, hyperparams["init_channels"], hyperparams["init_channels"]]
        )  # Input 2
...
class PerformancePredictor:
    ...
    def prepare_training_data(
        self, genomes: List[Dict], accuracies: List[float], latencies: List[float]
    ) -> DataLoader:
    ...
    def predict_performance(
        self, genomes: List[Dict], with_uncertainty: bool = True
    ) -> Dict[str, np.ndarray]:
        """
    ...
    def load_model(self, filepath: str):
        ...
        self.graph_converter = ArchitectureGraph(
            steps=config["steps"], num_ops=config["num_ops"]
        )
