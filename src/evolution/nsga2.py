...
class NSGA2:
    ...
    def __init__(self, population_size: int = 50, generations: int = 60,
        ...
        for obj_idx in range(num_objectives):
            ...
            for i in range(1, len(front) - 1):
                if self.objectives[obj_idx] == 'accuracy':
                    ...
                    neighbor_diff = (sorted_fitness[i-1, obj_idx] - sorted_fitness[i+1, obj_idx])
                else:
                    # For minimization
                    neighbor_diff = (sorted_fitness[i+1, obj_idx] - sorted_fitness[i-1, obj_idx])
    ...
    def binary_tournament_selection(self, population: List[Any], 
                                  fitness: np.ndarray, 
                                  fronts: List[List[int]]) -> List[Any]:
        """
    ...
    def evolve(self, population: List[Any], evaluate_func: Callable) -> Tuple[List[Any], Dict[str, Any]]:
        """
    ...
    def _select_new_population(self, population: List[Any], 
                             fitness: np.ndarray) -> Tuple[List[Any], np.ndarray]:
    ...
    def _update_history(self, population: List[Any], fitness: np.ndarray, generation: int):
    ...
    def _print_generation_stats(self, fitness: np.ndarray, generation: int):
        ...
        if len(pareto_front) > 0:
            ...
            print(f"  Accuracy range: {pareto_fitness[:, 0].min():.3f} - {pareto_fitness[:, 0].max():.3f}")
            print(f"  Latency range: {pareto_fitness[:, 1].min():.1f} - {pareto_fitness[:, 1].max():.1f} ms")
            print(f"  Params range: {pareto_fitness[:, 2].min():.1f} - {pareto_fitness[:, 2].max():.1f}M")
            print()
...
class PopulationManager:
    ...
    def evaluate_population(self, population: List[Dict], 
                          generation: int,
                          use_proxies: bool = True) -> np.ndarray:
        """
        ...
        if generation < 30 and use_proxies:
            ...
            for i, individual in enumerate(population):
                ...
                fitness[i] = [scores['accuracy_proxy'], 
                             scores['latency'], 
                             scores['params']]
