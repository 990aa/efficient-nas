...
class ShuffleNet(nn.Module):
    ...
    def _make_stage(self, inp: int, oup: int, repetitions: int, stride: int) -> nn.Sequential:
...
class ShuffleUnit(nn.Module):
    ...
    def __init__(self, inp: int, oup: int, stride: int, groups: int):
        ...
        self.branch2 = nn.Sequential(
            nn.Conv2d(inp if stride == 1 else mid_channels, mid_channels, 1, bias=False),
            nn.BatchNorm2d(mid_channels),
            nn.ReLU(inplace=True),
...
class BaselineManager:
    ...
    def evaluate_baseline(self, model_name: str, dataset: str = 'cifar10') -> Dict[str, Any]:
